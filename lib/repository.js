...
(function() {
  var Branch, Repository, Tree, TreeStore, addKeyPrefix, async, clone, commit, contains, findCommonCommit, findDiff, findDiffSince, findDiffWithPaths, intersection, keys, mergeDiffs, mergingCommit, objectDiff, objectDiffObject, read, readTreeAtPath, recurseUntilCommonCommit, splitCurrentAndChildTreeData, treeAncestors, treesAncestors, union, values, _, _ref,
    __slice = [].slice;
  ...
  union = _.union, values = _.values, keys = _.keys, intersection = _.intersection, clone = _.clone, contains = _.contains;
  ...
  findDiffSince = function(commonTreeHash, toTreeHash, knownTrees, treeStore) {
    var ancestor, diff, newCommonTreeHash, toTree, _i, _j, _len, _len1, _ref1, _ref2;
    ...
    if (toTree.ancestors.length === 1) {
      return mergeDiffs(diff, findDiffSince(commonTreeHash, toTree.ancestors[0], knownTrees, treeStore));
    } else if (toTree.ancestors.length === 0) {
      return mergeDiffs(diff, findDiff(null, toTreeHash, treeStore));
    } else {
  ...
  Repository = (function() {
    ...
    Repository.prototype.patchHashs = function(_arg) {
      ...
      for (_i = 0, _len = to.length; _i < _len; _i++) {
        ...
        diff = mergeDiffs(diff, findDiffSince(commonTree, eachTo, diff.trees, this.treeStore));
      }
      return diff;
    };
