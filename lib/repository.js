...
(function() {
  var Branch, Repository, Tree, TreeStore, addKeyPrefix, async, clone, commit, findCommonCommit, findDiff, findDiffSince, findDiffWithPaths, intersection, keys, mergingCommit, objectDiff, objectDiffObject, read, readTreeAtPath, splitCurrentAndChildTreeData, treeAncestors, treesAncestors, union, values, _, _ref,
    __slice = [].slice;
    ...
    if ((oldTrees.length === 0) || (positions.length === 0)) {
      ...
      reduceFun = function(diff, eachPosition) {
        ...
        if (ancestors.length > 0) {
          parentDiffReduceFun = function(diff, eachAncestor) {
            return mergeDiff(diff, findDiff(eachAncestor, eachPosition, treeStore));
          };
  ...
  Repository = (function() {
    ...
    Repository.prototype.patchHashsSince = function(trees1, trees2) {
      return findDiffSince(trees1, trees2, this.treeStore);
    };

    Repository.prototype.patchSince = function(trees1, trees2) {
      var diff, each;
      diff = this.patchHashsSince(trees1, trees2);
      diff.trees = (function() {
        var _i, _len, _ref1, _results;
        _ref1 = diff.trees;
        _results = [];
