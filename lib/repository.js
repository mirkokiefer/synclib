...
(function() {
  ...
  findDiffWithPaths = function(tree1Hash, tree2Hash, treeStore, cb) {
    ...
    return async.map([tree1Hash, tree2Hash], readOrCreateNewTree(treeStore), function(err, _arg) {
  ...
  Repository = (function() {
    ...
    Repository.prototype.deltaData = function(delta, cb) {
      var commits, obj, trees;
      obj = this;
      commits = function(cb) {
        return async.map(delta.commits, (function(each, cb) {
          return obj.commitStore.read(each, cb);
        }), cb);
      };
      trees = function(cb) {
        return async.map(delta.trees, (function(each, cb) {
          return obj.treeStore.read(each, cb);
        }), cb);
      };
      return async.parallel([commits, trees], function(err, _arg) {
        var commitsRes, treesRes;
        commitsRes = _arg[0], treesRes = _arg[1];
        return cb(null, {
          commits: commitsRes,
          trees: treesRes,
          values: delta.values
    ...
    Repository.prototype.merge = function(commit1, commit2, strategy, cb) {
      ...
      return this.commonCommit(commit1, commit2, function(err, commonCommit) {
        ...
        if (commit1 === commonCommit) {
          ...
          return async.map([commonCommit, commit1, commit2], readCommitTree, function(err, _arg) {
