...
(function() {
  ...
  Repository = (function() {
    ...
    function Repository(_arg) {
      var _ref1;
      _ref1 = _arg != null ? _arg : {}, this.treeStore = _ref1.treeStore, this.branchStore = _ref1.branchStore;
      if (!this.treeStore) {
        this.treeStore = contentAddressable();
      }
      if (!this.branchStore) {
        this.branchStore = keyValueStore();
      }
      this._treeStore = new TreeStore(this.treeStore);
      this._branches = {};
    }

    Repository.prototype.addBranch = function(name, hash) {
      if (hash) {
        this.branchStore.write(name, hash);
      }
      return this.branch(name);
    };

    Repository.prototype.removeBranch = function(name) {
      delete this._branches[name];
      return this.branchStore.remove(name);
    };

    Repository.prototype.branch = function(name) {
      var branch, cached, obj;
      if (cached = this._branches[name]) {
        return cached;
      } else {
        obj = this;
        branch = new Branch(this, this.branchStore.read(name));
        branch.on('postCommit', function(head) {
          return obj.branchStore.write(name, head);
        });
        return branch;
      }
    };

    Repository.prototype.detachedBranch = function(treeHash) {
