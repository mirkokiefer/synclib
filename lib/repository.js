...
(function() {
  var Branch, Repository, Tree, TreeStore, addKeyPrefix, async, clone, commit, contains, findCommonCommit, findDiffWithPaths, findPatch, findPatchDiff, intersection, keys, mergeDiffs, mergingCommit, objectDiff, objectDiffObject, read, readTreeAtPath, recurseUntilCommonCommit, splitCurrentAndChildTreeData, treeAncestors, treesAncestors, union, values, _, _ref,
    __slice = [].slice;
  ...
  findPatchDiff = function(tree1Hash, tree2Hash, treeStore) {
  ...
  findPatch = function(commonTreeHash, toTreeHash, knownTrees, treeStore) {
    ...
    if (toTree.ancestors.length === 1) {
      return mergeDiffs(diff, findPatch(commonTreeHash, toTree.ancestors[0], knownTrees, treeStore));
    } else if (toTree.ancestors.length === 0) {
      return mergeDiffs(diff, findPatchDiff(null, toTreeHash, treeStore));
    } else {
  ...
  Repository = (function() {
    ...
    Repository.prototype.patchHashs = function(_arg) {
      ...
      for (_i = 0, _len = to.length; _i < _len; _i++) {
        ...
        diff = mergeDiffs(diff, findPatch(commonTree, eachTo, diff.trees, this.treeStore));
      }
      return diff;
    };
