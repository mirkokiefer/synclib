...
(function() {
  ...
  findPatch = function(commonTreeHash, toTreeHash, knownTrees, treeStore) {
    ...
    ancestorDiffs = (function() {
      ...
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        ancestor = _ref1[_i];
        _results.push(findPatchDiff(ancestor, toTreeHash, treeStore));
      }
      return _results;
    })();
    if (toTree.ancestors.length === 1) {
      return mergeDiffs(ancestorDiffs[0], findPatch(commonTreeHash, toTree.ancestors[0], knownTrees, treeStore));
    } else if (toTree.ancestors.length === 0) {
      return findPatchDiff(null, toTreeHash, treeStore);
    } else {
      diff = {
      ...
      reduceFun = function(diff, ancestor) {
        ...
        return mergeDiffs(diff, findPatch(newCommonTreeHash, ancestor, knownTrees, treeStore));
      };
